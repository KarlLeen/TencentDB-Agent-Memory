...
**109 · D6 证伪（A 判据 = 逐字连续重合 ≥ `L_q`）**：夹具 `citation/__tests__/fixtures/d6-falsification-cases.json`
（R-1 模板异内容 / R-2 同主题独立 / R-3 通用句式 = 合成且**规则先冻结**；R-4 = 真库**真实未读过**消息 28 条、
只存元数据）+ 同一入口（`npx tsx scripts/qa/shadow-calibration.ts`，含语料冻结指纹校验）。
**读数（只报数）**：真实未读 28 条逐字重合 max **3** ⇒ **`L_q ≥ 8` 全部假阳 0**；R-2 ≤ 3；R-1 = 23（21 = 模板前缀、
...
- **（87 append；2026-09-12，append-only）实测基线**：`scripts/qa/throughput-measure.ts`（显式临时库 + 假 judge
  `mock:v1`，**零 LLM 花费**；**mock 耗时 ≠ 真 provider 耗时**——mock 数据只代表框架吞吐）。**实测（12 轮/组）**：
  到达 ≤ N/轮 ⇒ pending 归零（`30→0`）；到达 = 2N ⇒ **pending 单调 +N/轮**（`30→360`，与 `a−N` 精确一致）。
  **N/T 敏感性**：每轮产能 ≈ `topN / T`（`T∈{2,5,10}s`：`topN=30 ⇒ 15/6/3`、`100 ⇒ 50/20/10` units/s）。
  **判据建议**：`pending` 连续 `K` 轮超过 `X`（建议 `K=3`、`X=2×topN`——依据：实测双倍到达档 3 轮即破 60）⇒ 视为不匹配。
  **处置四选项（只建议，不实施）**：① 提高 `topNPerCycle`（成本最低、只抬上限不改语义）；② 批间并行（复杂度↑、须先证 FIFO 不饿死语义）；③ 写入侧 backpressure（改写入路径、影响面最大）；④ 接受 + 告警（对照项）。

  **适用范围（88 append；2026-09-12，append-only）**：上文"到达 ≤ N/轮 ⇒ pending 归零（`30→0`）"**仅在该组
  「起点 pending = 0」时成立**——本脚本**各组共享同一临时库**（生产语义上更真实：积压连续），故**后续组的
  "≤N"档带着前组积压**：复核实测 `topN=50 到达=25` = **335→60**、`topN=100 到达=50` = **610→60**
  （`单调增=false`，12 轮不足以排空）。⇒ 正确表述：**无积压起点 ⇒ 归零；带积压起点 ⇒ 单调下降 / 清空过程**。

## 18 消费侧归属判定与 S5 交付对账（§9.5 第二半；63 落地）

> 本节是 §9 第 5 项的后半 + **S5 收尾**：对 `attribution_audit` 池 / `asset_validated` /
> `asset_corrected` 三件**先判归属、再动手**。判据（三条，写死）：
> ① **有 S5 内可点名的生产者？**（生产者不在 S5 ⇒ 不进实现）
> ② **语义/形状是否依赖未定稿的设计？**（依赖 ⇒ 现在建就是自造漂移源）
> ③ **谁读？**（写不出"谁读" ⇒ 不得实现——陷阱 14：有字段 ≠ 被读出）

### 18.1 三件逐件判定（证据 → 结论 → 处置）

**① `asset_corrected` 写口 → 判：S6（登记不建）**
...
- 三条判据：① **生产者为空**（连"什么事件产生 validated"都未定义）；② 语义完全未定稿；③ 无可写出的读者。
- 处置：**登记不建**（**不许"先建表占位"**——无生产者/无读者的表就是 59/60/62 一路在防的死码）。
  触发条件 = 其产生条件被某设计定稿时，与写口同单落地。

**③ `attribution_audit` 池 → 判：后置（S7 展示层依赖，登记不建）**
- 证据（三条，按硬度排序）：
  a. **`00-master-spec.md:113`**——"v2：**回执 + 抽查池两页（落地⑤）| S7 | MemoryPanel**"（池的**用户面（两页 UI）明文在 S7**）；
  b. `attribution-v2-review.md:108`——"抽查/分歧池（**含 `unconfirmed_suspect`**，**只服务展示层**）"；`unconfirmed_suspect` 全仓仅此半句，**"suspect"的判据无定义**；
  c. `attribution-base-design.md:57`——该表"50 spec 前定稿"（见 18.3 的改期登记）。
- 三条判据逐条：① 生产者**候选**可点名（判定的 unconfirmed 结果）——**但**其真实数据**已全落** `judgement_details`（`verdict='unconfirmed'` 行；既有 `listByUnit`/`listBySession` 可查）⇒ **不建池零信息损失**；② **不过**——池独有的新增语义（"suspect"判据、抽查状态、分歧标记）全仓仅一句命名、**依赖展示层设计（未定稿）**⇒ 现在建 = 自造漂移源；③ **不过**——读者 = 展示层（S7 未到），"运维 SQL 查 unconfirmed"这一最小读者**已被既有查询口满足**（无需新表）。
- 处置：**登记不建**。触发条件 = **S7 展示层落地（"回执 + 抽查池两页"）时同批定稿 + 建表**；或抽查/分歧需求先以独立设计出现时同理。

> **S7 前置阅读（68 append）**：回执 / 抽查池开工前必读 60 spec §5 的"payload 新鲜度契约"——
> corrected 行的版本字段是**检测时快照**，不得当"当前版本"渲染。

### 18.2 本单实现 = 空（零生产代码声明）

三件全判后置 ⇒ **无判属 S5 的部分可写**。本单交付 = 本节判定 + 下方对账表（照 58"停在标定"先例：
**判完无码可写是合法交付**；**不为让工单看起来有产出而建表**）。
...
### 18.4 §14"validated/corrected 禁写"断言的处理
...
| §9 项 | 交付单元 | commit | 验收锚点 |
|---|---|---|---|
| ① 决策单元锚定（fetched → 决策单元） | 51（brief/P-0）+ 52（会话键归一）+ **55**（锚定本体） | `f1df854` / `9a342d6` | §10 C1–C5；15 格同键矩阵 + R1/R2；P-0a 真库复测（47 行、倒挂 0） |
| ② 候选分档 + shortlist + 全文核实 + 三道机械锚点 | **56** + **57** + 58（B5 标定/裁决） | `441343e` / `dd0ef36` / `ce150a5` | §11（供给）/ §12（shortlist+度量）/ §13（mechanical:v1）；56 T7 e2e、57 T6 e2e、58 T5 e2e |
...
**S5 结论**：§9 五项**主体全部交付**（①–⑤ 各有 commit + 验收锚点）；三项**显式后置**均有
触发条件（`validated`/`audit` → S6/S7 依赖；`task_boundary`/corrected → S6）——**S5 至此整体收口**。

---

## 19 S6 后续指针（append-only；本档无勘正区，以本节替代指针）

- **S6 全部设计定稿 = `60-corrected-rules.md`**（64 落档；零生产代码）：corrected 三词术语
  （§1）/ 三路机器规则（§2）/ 版本链快照（§3）/ `task_boundary` 启发式 + **本节 §16.3 与
  §14 的收窄动作清单**（§4、§5）/ 消费面（§6）/ 验收含 O10 扩样（§7）/ 风险与缺口（§8/§9）。
- **对本档的后续动作**（由 S6 实现单同批执行，**本节只登记，不改 §10–§18 原行**）：
