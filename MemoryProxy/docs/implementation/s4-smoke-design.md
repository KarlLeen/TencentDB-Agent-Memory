...
- **计数纪律（85 append；2026-09-12，append-only）**：本系列的**工单与报告**引用**任何计数 / 度量**
  时必须**同时给出取样口径**——**① 范围**（在什么集合里数的）+ **② 排除项**（排除了什么）+
  **③ 现取命令**；三项缺一 ⇒ 该数字**不算证据**。最小合规模板：
  `X（范围 = …；排除 = …；命令 = …）`。
  **三例实证（全部发生在 2026-09-12）**：① **`grep -c '^-[^-]'` 计数删除行漏计**以 `-` / `--` 开头的行
  （markdown 列表项删除渲染成 `-- …`）⇒ 已改判"**以 `--numstat` 删除列为准**"（82 append 2）；
  ② **`grep -v __tests__` 之后报"源码 0 命中"**（`asset_validated`）——**全量实为 1 处**
  （`corrected-rules.test.ts:170` 的**禁写验证**：断言 `failed` + 零行），正确表述 =
  "**0 处生产者 + 1 处禁写验证**"；③ **前缀长度用了过期常量 862**（应为**当场现取**的 867）⇒ 误报
...
- **S8-b 键口径缺口 + 开闸 no-op（99 append；2026-09-12，append-only）**：S8-b 的查/渲键 = 行级 scene `path`，
  而事件面的资产 = **整段 `memoryAssetId`**（两个不同粒度、不同来源的键空间 ⇒ 永不命中）⇒ **开闸为 no-op**
  （信用列 / 标黄不出现、精排保持原序；不报错、不错排，只是什么都没读到）。根因两条：(a) **spec gap**
  （契约未指定键空间）；(b) **测试设计盲区**（T1/T2 直接注入合成 `L2RankingData`、从不经过真实键空间 ⇒
  绿而不证）。**处置**：诚实声明 + 口径一致性测试（`T7`，按**真实生产者键**喂 ⇒ 与关闸逐字节相同）见
  `80-credit-score-and-ranking.md` §4.1；**方案 A**（行级归属；跨切片）**挂起**（触发 = 拍定"行级资产身份"）；
  **方案 B**（段级口径）**明确否掉**（误导性展示）；S8-d 触发判据补"**必须含行级归属落地**"（防验收 no-op）。
- **可查性论证须连带查"唯一性"（随单条；2026-09-12，append-only）**：凡以"某语句在文档内**唯一/可查**"
  为论证（如"该句是唯一载体"），必须**连带断言其唯一出现**（`grep -c` 计数 = 1）；否则论证强度被高估
  ——`91` 删 `503` 字样**首演未红**即因该语句在文件内出现**两处**（正文段 + 表行），删一处不红。
- **接线单必须走真实取数路径（随单条；2026-09-12，append-only）**：凡单子里出现"消费 X 的数据"，
  必须**至少一格测试走真实取数路径**（从生产者产出的键/形状出发），**不得**只用"直接注入合成输入"的
  单测当作接线已成立的证据 —— 后者只证明**消费端逻辑本身**没错。（同一模式的第 3–4 次重演：
  golden 测试绕过 `consumeRow`、e2e 绕过 `anthropicHandler`、`98` 的 T1/T2 直接注入合成
  `L2RankingData` 而从不经过 `loadL2Ranking` 的真实键空间 ⇒ 绿而不证，掩住"键不同源 ⇒ 开闸 no-op"。）
- **`#6` 残项登记（随单条；2026-09-12，append-only）**：`98`/`99` 的 `T7` 目前只做到**键空间忠实**
  （按真实生产者键 `memoryAssetId` 喂数据 ⇒ 证明 no-op），**尚未**是"端到端取数路径"——`loadL2Ranking`
  **未导出**，测试无法经过它。**补齐时机 = 方案 A / S8-d 触发时同批**（导出 loader + 一格真实库驱动测试）。
- **计数/匹配型判据"口径先行"（随单条；2026-09-12，append-only）**：凡以**计数或匹配**作判据
  （`grep -c`、`--numstat` 删除列、路径/键空间对照等），必须**先在已知项上校验该计数法**（例：某键的
  已知期望值），**校验通过后才用于全表**；并须**声明覆盖范围**（如"`src/` 内"而非"全仓"）与**已知漏计
  形态**（`grep '^-[^-]'` 漏 `--` 开头行）。反例合订：`91`（唯一性未连带）、`81`（删除行漏计）、
  `100`（**正例**：`storage = 115` 先校验后全表）。
- **`#6` 残项已提前补齐（随单条；2026-09-12，append-only）**：`102` 已导出 `loadL2Ranking`（仅加 `export`，
...
- **影子"来源"字段（`116` P3 修复；2026-09-13，append-only）**：`121` 增
  ...
  `du_dd46*`/`du_22e8*`/`du_c98a*` ⇒ **各 13×`block`**；**既有字段与 116 基线逐字一致（不符 0）**、
  (seg/run) 分布 12×seg1 + 1×seg67 全保留。⇒ "同形不可分"两形态（无文本 / 有文本但过短无片段）**可分**；
  **禁止**用 `segCount === 0` 反推（R1）；池不接入（D4）。
- **D6 证伪实验：A 判据的 FP/FN 曲线（随单条；2026-09-12，append-only）**：`109` 以四类反例测
  A 判据（逐字连续重合 ≥ `L_q`，`L_q ∈ {8,12,16,24,32,48}`）：**R-4 真实未读 28 条 run ∈ {2,3}**
  （**任何 `L_q ≥ 8` 假阳 0**）、R-2 ≤ 3、R-1 = **23**（21 = 模板前缀 `Background + SOP for `，
  +2 = 句式片段 —— **构造时的记忆污染如实登记**）、R-3 = [8,12,5,16,23,16]；正例（整行/半行）
  在所有 `L_q ≤ 48` 均 3/3 + 3/3（**FN = 0**）；**HN3 在 `L_q ≤ 24` 全中**（"改写"被算引用 = **已知代价**）。
