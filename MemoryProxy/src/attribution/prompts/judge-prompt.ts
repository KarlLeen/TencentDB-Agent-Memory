...
export const ATTRIBUTION_JUDGE_PROMPT_V2: JudgePromptTemplate = {
  ...
  text: [
    ...
    "",
    'A "turn_context" field (when present) lists OTHER items in the SAME turn, in',
    "order. Items with kind key_tool_call/code_change are decision units; items with",
    "kind tool_call are auxiliary shell calls (background material, NOT decisions).",
    "Use the whole sequence only to understand whether the current unit is part of a",
    "multi-step decision (e.g. run tests -> grep failures -> diff baseline); the",
    "verdict still targets the CURRENT unit and a SINGLE asset.",
    "",
    "Reply with exactly one JSON object:",
    '{"asset_id": string|null, "verdict": "confirmed"|"refuted"|"unconfirmed", "rationale_ref": string}',
    "",
