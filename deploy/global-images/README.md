...
2. **单独起 worker 进程**（与 proxy **同库**、常驻轮询）：
   ...
   ```bash
   # 源码部署：仓库内直接跑
   npm run worker:attribution
   # 容器化部署：镜像内保留完整应用与运行入口（见 MemoryProxy/Dockerfile），在 proxy 容器内执行同一条入口
   docker exec -d -w /app tdai-proxy npm run worker:attribution
   ```

   运维旗标：`--once`（抽干一轮后退出）、`--retry-failed`（死信复位）、`--correct-l1`（L1 自动接线；默认关）。

面板侧凭证（`ATTRIBUTION_PROXY_ADMIN_KEY` 等）见 `deploy/panel-knowledge-combined/README.md` 的「归因面板（Attribution）」节。**worker 是否容器化（独立容器 / 服务）属产品决定，本仓未内置。**
